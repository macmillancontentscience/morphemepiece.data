...
.make_lookup_list <- function(voc, lu, word_frequency_table = NULL) {
  ...
  if (!is.null(word_frequency_table)) {
    ...
    lu <- lu %>%
      dplyr::left_join(word_frequency_table, by = "word") %>%
      dplyr::arrange(dplyr::desc(.data$word_count))
  }

  # The lookup_list is the first 2 columns, simplified.
  lookup_list <- lu$tokenization
  names(lookup_list) <- lu$word
